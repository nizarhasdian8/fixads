...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold text-stone-900">Edit Produk</h1>
</div>

<form method="POST" action="{{ route('produk.update', $produk) }}" class="max-w-2xl">
    @csrf
    @method('PUT')
    <div class="bg-white border border-stone-200 rounded-2xl p-6 space-y-5">
        <div class="grid sm:grid-cols-2 gap-5">
            ...
            <div class="sm:col-span-2">
                ...
                <textarea name="deskripsi" rows="3" class="w-full px-3.5 py-2.5 rounded-xl border border-stone-300 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition">{{ old('deskripsi', $produk->deskripsi) }}</textarea>
            </div>
            <div class="sm:col-span-2">
                <label class="flex items-center gap-2 cursor-pointer select-none">
                    <input type="checkbox" name="status_aktif" value="1" @checked(old('status_aktif', $produk->status_aktif)) class="rounded border-stone-300 text-brand-600 focus:ring-brand-500">
                    <span class="text-sm text-stone-700">Produk aktif (tampil sebagai pilihan saat input pesanan)</span>
                </label>
            </div>
        </div>
    </div>
    ...
    <div class="flex items-center justify-end gap-3 mt-6">
        ...
        <button type="submit" class="px-5 py-2.5 rounded-xl bg-brand-500 hover:bg-brand-600 text-white font-semibold text-sm transition shadow-sm shadow-brand-500/20">Simpan Perubahan</button>
    </div>
</form>
@endsection
