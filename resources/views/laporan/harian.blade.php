...
@section('title', 'Laporan Harian Bahan Baku')

@section('content')
<div class="mb-6 flex flex-wrap items-center justify-between gap-4">
    <div>
        <h1 class="text-2xl font-bold text-stone-900">Laporan Harian Bahan Baku</h1>
        <p class="text-stone-500 text-sm mt-1">Rekap data bahan baku yang masuk dan keluar per hari.</p>
    </div>
    <a href="{{ route('laporan.harian.pdf', ['tanggal' => $tanggal]) }}" target="_blank" class="inline-flex items-center gap-2 px-4 py-2.5 bg-red-500 hover:bg-red-600 text-white font-semibold text-sm rounded-xl transition shadow-sm">
...
<div class="bg-white border border-stone-200 rounded-2xl p-5 mb-6">
    <form method="GET" action="{{ route('laporan.harian') }}" class="flex items-end gap-4">
        <div>
            <label class="block text-sm font-medium text-stone-700 mb-1.5">Pilih Tanggal</label>
            <input type="date" name="tanggal" value="{{ $tanggal }}" required class="px-3.5 py-2.5 rounded-xl border border-stone-300 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500 transition">
        </div>
            ...
            <table class="w-full text-sm text-left">
                <thead class="bg-stone-50 text-stone-500 uppercase text-xs">
                    <tr>
                        ...
                        <th class="px-5 py-3 font-medium">Untuk Pesanan</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-stone-100">
                    ...
                    @empty
                    <tr>
                        <td colspan="3" class="px-5 py-8 text-center text-stone-400">Tidak ada bahan keluar pada tanggal ini.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-sm text-left">
                ...
                <tbody class="divide-y divide-stone-100">
                    ...
                    @empty
                    <tr>
                        <td colspan="3" class="px-5 py-8 text-center text-stone-400">Tidak ada bahan masuk pada tanggal ini.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
